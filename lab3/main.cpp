...
auto brute_force = [](auto f, int iterations, double minimal_d, double maximal_d) {
    ...
    double precision = (maximal_d - minimal_d) / (iterations/10);
...
auto simulated_annealing = [](auto f, int max_iterations, double minimal_d, double maximal_d) {
    ...
    for (int i = 0; i < max_iterations; ++i) {
        auto tk = coords_generator(minimal_d, maximal_d);
...
//    std::vector<std::pair<double, double>> domain_range = {{-10, 10},{-10, 10},{-5,  5}};
//    std::vector<std::string> algorithmsNames = {"Brute Force", "Hill Climbing", "Simulated Annealing"};

//
    printResults(brute_force, "Brute Force", boothFunc, "Booth", {-10, 10}, 100000);
    printResults(brute_force, "Brute Force", holderTableFunc, "Holder table", {-10, 10}, 100000);
    printResults(brute_force, "Brute Force", treeHumpCamelFunc, "Tree Hump Camel", {-5, 5}, 100000);

    std::cout << std::endl;

    printResults(hill_climbing, "Hill Climbing", boothFunc, "Booth", {-10, 10}, 100000);
    printResults(hill_climbing, "Hill Climbing", holderTableFunc, "Holder table", {-10, 10}, 100000);
    printResults(hill_climbing, "Hill Climbing", treeHumpCamelFunc, "Tree Hump Camel", {-5, 5}, 100000);

    std::cout << std::endl;

    printResults(simulated_annealing, "Simulated Annealing", boothFunc, "Booth", {-10, 10}, 100000);
    printResults(simulated_annealing, "Simulated Annealing", holderTableFunc, "Holder table", {-10, 10}, 100000);
    printResults(simulated_annealing, "Simulated Annealing", treeHumpCamelFunc, "Tree Hump Camel", {-5, 5}, 100000);
